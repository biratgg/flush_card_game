...
import os
import math
from shuffle import shuffle_and_deal   # uses the fixed shuffle.py
...
def num_players_input():
    global NUM_PLAYERS
    NUM_PLAYERS = int(input("enter the number of players: ") ) 
    names=[]

    if NUM_PLAYERS >18:
        print("maximum no. of players is 17")
        num_players_input()
    elif NUM_PLAYERS <2:
        print("minimun no. of players is 2")
        num_players_input()
    else:
        for i in range(NUM_PLAYERS):
            names.append(input(f"enter the names of player {i+1} "))
